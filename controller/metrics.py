...
model_switches_total = Counter(
    'vllm_studio_model_switches_total',
    'Total number of model switches',
    ['recipe_id', 'backend']
)

model_switch_duration_seconds = Histogram(
...
active_model_info = Info(
    'vllm_studio_active_model',
    'Currently active model information'
)

inference_server_up = Gauge(
    'vllm_studio_inference_server_up',
    'Whether inference server is running (1=up, 0=down)'
)

# GPU metrics
gpu_memory_used_bytes = Gauge(
    'vllm_studio_gpu_memory_used_bytes',
    'GPU memory used in bytes',
    ['gpu_id', 'gpu_name']
)

gpu_memory_total_bytes = Gauge(
    'vllm_studio_gpu_memory_total_bytes',
    'Total GPU memory in bytes',
    ['gpu_id', 'gpu_name']
)

gpu_utilization_percent = Gauge(
    'vllm_studio_gpu_utilization_percent',
    'GPU utilization percentage',
    ['gpu_id', 'gpu_name']
)

gpu_temperature_celsius = Gauge(
    'vllm_studio_gpu_temperature_celsius',
...
sse_active_connections = Gauge(
    'vllm_studio_sse_active_connections',
    'Number of active SSE connections',
    ['channel']
)

sse_events_published_total = Counter(
    'vllm_studio_sse_events_published_total',
...
def update_active_model(model_path: str = None, backend: str = None, served_name: str = None):
